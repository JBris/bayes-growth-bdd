...
Feature: Nonlinear growth model for Australian blacktip sharks (Carcharhinus tilstoni)
    Background:
        ...
        And we define parameter "L_inf" ("cm") as "The theoretical upper asymptotic size for infinite growth."
        And we define parameter "k" ("years^-1") as "The growth coefficient representing the rate of growth."
    ...
    @fisheries_modelling
    Scenario: Fit a von Bertalanffy growth model for male Australian blacktip sharks (Carcharhinus tilstoni)
        ...
        And we believe that the "L_inf" parameter could plausibly be "156.8" with a standard deviation of "10.0" and a "lower" bound of "0.0"
        And we believe that the "k" parameter could plausibly be "0.25" with a standard deviation of "0.2" and a "lower" bound of "0.0"
    ...
    @fisheries_modelling
    Scenario: Fit a biphasic von Bertalanffy growth model for male Australian blacktip sharks (Carcharhinus tilstoni)
        ...
        And we believe that the "k" parameter could plausibly be "0.25" with a standard deviation of "0.2" and a "lower" bound of "0.0"
        And we believe that the "t_0" parameter could plausibly be "0.0" with a standard deviation of "2.0"
        And we believe that the "t_h" parameter could plausibly be "3.5" with a standard deviation of "0.75" and a "lower" bound of "0.0"
    ...
    Scenario: Fit a von Bertalanffy growth model for female Australian blacktip sharks (Carcharhinus tilstoni)
        ...
        And our response variable is "Total Length" ("cm")
        And our explanatory variable is "Age" ("years")
        And we believe that the "L_inf" parameter could plausibly be "181.4" with a standard deviation of "10.0" and a "lower" bound of "0.0"
        And we believe that the "k" parameter could plausibly be "0.19" with a standard deviation of "0.15" and a "lower" bound of "0.0"
        And we believe that the "t_0" parameter could plausibly be "0.0" with a standard deviation of "2.0"
        And we fit random intercepts to "year"
        And we fit random intercepts for "year" to "L_inf"
        And we aim to evaluate the "0.95" highest posterior density intervals (HDIs) of our parameter estimates
    ...
    @fisheries_modelling
    Scenario: Fit a biphasic von Bertalanffy growth model for female Australian blacktip sharks (Carcharhinus tilstoni)
        Given our growth curve is a "biphasic von Bertalanffy growth model (Soriano et al., 1992)" ("BVBGM")
        And our sex is "Female"
        And we have samples taken from "New South Wales A, and Queensland A"
        And we have samples taken between "2007" and "2012"
